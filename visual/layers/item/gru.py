...
class VGRU(VItem):
    def __init__(self, logic, scene, x, w_info, w_flat, w_volume):
        ...
        if HintsKeeper().display == Display.COMPACT:
            ...
            self._block = VGRUBlock(self._scene, self._x, self.select, self.show_output)

        elif HintsKeeper().display == Display.EXTENDED:
            self._connection = LinkType.SEPARATED
            self._neuron_ctrl = VGRUNeuronController(self._scene, self._x, self._logic.units, self.select, self.show_output, logic)
