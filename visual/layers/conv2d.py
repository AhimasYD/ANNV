...
class VConv2D(VLayer):
    def __init__(self, logic, scene, x, o_display, o_color, o_thick, o_names, o_captions, o_bias, w_info, w_flat, w_volume):
        ...
        if self._o_display == Display.COMPACT:
            self.block = VConv2DBlock(self._scene, self._x, self.select, self._o_names)
            ...
            for i in range(channels):
                self.kernels[i] = VConv2DKernel(self._scene, self._logic.filters[self.filter, i], self._x, 0, self.select)
